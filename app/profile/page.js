        ...
        <div className="py-4 px-2">
            <div className="hidden sm:block">
                <div className="relative flex items-center justify-between">
                    ...
                    <div className="absolute left-0 top-5 h-1 bg-gradient-to-r from-brand-orange to-orange-400 rounded-full transition-all duration-500" style={{ width: `${((Math.min(status, 4) - 1) / 3) * 100}%` }} />
                    {timelineStages.map((stage) => {
                        ...
                        return (
                            <div key={stage.id} className="relative flex flex-col items-center z-10">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isCompleted ? "bg-gradient-to-br from-brand-orange to-orange-500 text-white shadow-lg" : "bg-white border-2 border-gray-300 text-gray-400"} ${isCurrent ? "ring-4 ring-orange-100 scale-110" : ""}`}>
                                ...
                                <div className={`mt-3 flex flex-col items-center ${isCompleted ? "text-gray-900" : "text-gray-400"}`}>
                                    <StageIcon className={`w-5 h-5 mb-1 ${isCompleted ? "text-brand-orange" : ""}`} />
                    ...
                    return (
                        <div key={stage.id} className="flex items-start gap-3">
                            <div className="flex flex-col items-center">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isCompleted ? "bg-gradient-to-br from-brand-orange to-orange-500 text-white" : "bg-white border-2 border-gray-300 text-gray-400"} ${isCurrent ? "ring-2 ring-orange-100" : ""}`}>
                                    {isCompleted ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs">{stage.id}</span>}
                                </div>
                                {!isLast && <div className={`w-0.5 h-6 ${isCompleted ? "bg-brand-orange" : "bg-gray-200"}`} />}
                            </div>
                            <div className={`flex items-center gap-2 pt-1 ${isCompleted ? "text-gray-900" : "text-gray-400"}`}>
                                <StageIcon className={`w-4 h-4 ${isCompleted ? "text-brand-orange" : ""}`} />
                    ...
                    <aside className={`fixed lg:static top-0 left-0 w-72 bg-white z-[70] lg:z-auto transform lg:transform-none transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} lg:block flex-shrink-0 h-screen lg:h-auto`}>
                        <div className="bg-white lg:rounded-2xl shadow-lg lg:sticky lg:top-24 h-full lg:h-auto flex flex-col overflow-hidden">
                            ...
                            <div className="p-5 bg-gradient-to-r from-gray-900 to-gray-800 hidden lg:block">
                                <Link href="/" className="flex items-center"><span className="text-xl font-extrabold text-white">Cellfix<span className="text-brand-blue">BD</span></span></Link>
                            </div>
                                            ...
                                            <div key={item.id}>
                                                ...
                                                <button onClick={() => handleNavClick(item.id)}
                                                    className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center gap-3 mb-1 ${activeSection === item.id
                                                        ? (item.id === "dashboard" ? "bg-gradient-to-r from-brand-orange to-orange-500 text-white shadow-lg shadow-orange-500/30" : "bg-white text-brand-orange shadow-sm")
                            ...
                            <>
                                <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 rounded-2xl p-4 md:p-6 mb-4 md:mb-6 flex items-center gap-4 md:gap-6 shadow-xl">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <span className="inline-block mt-1 px-2 py-0.5 bg-brand-orange text-white text-[10px] md:text-xs font-bold rounded-full">MEMBER</span>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5">
                                    {[
                                        ...
                                        { id: "coupons", label: "Coupons", desc: "Available discounts", gradient: "from-amber-500 to-orange-500", shadow: "shadow-amber-500/30", icon: <Tag className="w-6 h-6 md:w-7 md:h-7 text-white" /> },
                                ...
                                <div className="border-b overflow-x-auto bg-gray-50">
                                    <div className="flex">
                                        {ORDER_TABS.map(tab => {
                                            ...
                                            return (<button key={tab.id} onClick={() => setActiveOrderTab(tab.id)} className={`flex items-center gap-2 px-4 md:px-6 py-4 text-xs md:text-sm font-medium whitespace-nowrap border-b-2 transition-all ${activeOrderTab === tab.id ? "border-brand-orange text-brand-orange bg-white" : "border-transparent text-gray-500 hover:text-gray-700"}`}><TabIcon size={16} />{tab.label}</button>);
                                        })}
                                    </div>
                                </div>
                                <div className="p-4 md:p-6">
                                    {ordersLoading ? (
                                        <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-brand-orange" /></div>
                                    ) : orders.length > 0 ? (
                                                ...
                                                <div key={order.id} className="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-lg transition-all group">
                                                    <div className="flex gap-4">
                                                        ...
                                                        <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
                                                            <div>
                                                                <div className="flex justify-between items-start gap-2">
                                                                    ...
                                                                    <div className="text-right flex-shrink-0">
                                                                        <p className="text-lg md:text-xl font-bold text-brand-orange">৳{(Number(order.sub_total ?? order.total ?? 0) + Number(order.delivery_fee ?? 0))}</p>
                                                            ...
                                                            <button onClick={() => setSelectedOrder(order)} className="mt-3 w-full py-2 px-3 bg-gray-100 hover:bg-brand-orange hover:text-white text-gray-700 text-xs font-semibold rounded-lg transition-all">View Details</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
                                            ...
                                            <div className="p-6 space-y-6">
                                                    ...
                                                    <div className="space-y-3">
                                                        {selectedOrder.sales_details?.map((item, i) => (
                                                            <div key={i} className="flex gap-4 p-3 bg-gray-50 rounded-xl">
                                                                ...
                                                                <div className="text-right"><p className="font-bold text-brand-orange">৳{item.price * item.qty}</p></div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                                                <div className="p-4 bg-gray-50 rounded-xl space-y-2 text-sm">
                                                    ...
                                                    <div className="flex justify-between pt-2 border-t font-bold text-lg"><span>Total</span><span className="text-brand-orange">৳{(Number(selectedOrder.sub_total ?? selectedOrder.total ?? 0) + Number(selectedOrder.delivery_fee ?? 0) - Number(selectedOrder.coupon_discount ?? 0))}</span></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                                ...
                                <div className="p-6">
                                    <form onSubmit={handleTrackOrder} className="flex gap-3 mb-6">
                                        <input type="text" value={trackInvoiceId} onChange={e => setTrackInvoiceId(e.target.value)} placeholder="Enter Invoice ID (e.g. INV-12345)" className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange text-sm" />
                                        <button type="submit" disabled={trackLoading} className="px-6 py-3 bg-brand-orange text-white font-bold rounded-xl hover:bg-[#ff1a2b] transition-colors disabled:opacity-70">
                                    ...
                                    {couponsLoading ? (
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {coupons.map((coupon, i) => (
                                                <div key={i} className="border border-dashed border-orange-300 rounded-xl p-4 bg-orange-50/50 hover:shadow-md transition-all">
                                                    <div className="flex items-center justify-between mb-2">
                                                        <span className="px-3 py-1 bg-brand-orange text-white text-xs font-bold rounded-full">{coupon.coupon_code || coupon.code}</span>
                                                        <span className="text-lg font-extrabold text-brand-orange">{coupon.discount_type === "percentage" ? `${coupon.discount}%` : `৳${coupon.discount}`}</span>
                                                    </div>
                                    ...
                                    {isEditing ? (
                                        <form onSubmit={handleProfileUpdate} className="space-y-5">
                                            <div className="flex justify-center mb-4">
                                                <div className="relative">
                                                    <div className="w-24 h-24 rounded-full bg-gray-200 overflow-hidden ring-4 ring-brand-orange/20">
                                                    ...
                                                    <label className="absolute bottom-0 right-0 w-8 h-8 bg-brand-orange text-white rounded-full flex items-center justify-center cursor-pointer shadow-lg hover:bg-[#ff1a2b] transition-colors">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <input type="text" value={formData.first_name} onChange={e => setFormData({ ...formData, first_name: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange text-sm" />
                                                </div>
                                                <div>
                                                    <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wider">Last Name</label>
                                                    <input type="text" value={formData.last_name} onChange={e => setFormData({ ...formData, last_name: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange text-sm" />
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wider">Email</label>
                                                <input type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange text-sm" />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wider">Phone</label>
                                                <input type="tel" value={formData.mobile_number} onChange={e => setFormData({ ...formData, mobile_number: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange text-sm" />
                                            </div>
                                            <div>
                                                <label className="block text-xs font-semibold text-gray-700 mb-1.5 uppercase tracking-wider">Address</label>
                                                <textarea value={formData.address} onChange={e => setFormData({ ...formData, address: e.target.value })} rows={3} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange text-sm resize-none" />
                                            </div>
                                            <div className="flex gap-3">
                                                ...
                                                <button type="submit" disabled={isUpdating} className="flex-1 py-3 bg-brand-orange text-white font-bold rounded-xl shadow-lg shadow-brand-orange/30 hover:bg-[#ff1a2b] transition-all disabled:opacity-70">
                                        ...
                                        <div className="space-y-6">
                                            <div className="flex items-center gap-4 mb-6">
                                                <div className="w-20 h-20 rounded-full bg-gray-200 overflow-hidden ring-4 ring-brand-orange/20 flex-shrink-0">
