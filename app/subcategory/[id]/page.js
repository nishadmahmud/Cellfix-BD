...
import { getProductsBySubcategory, getCategoriesFromServer } from '../../../lib/api';
...
export default function SubcategoryPage() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            // Try to resolve name from categories
            try {
                const catRes = await getCategoriesFromServer();
                if (catRes?.success && Array.isArray(catRes.data)) {
                    for (const cat of catRes.data) {
                        const found = (cat.sub_category || []).find(s => String(s.id) === String(subcategoryId));
                        if (found) {
                            if (isMounted) setSubcategoryName(found.name);
                            break;
                        }
                    }
                }
            } catch (e) {
                console.error('Failed to resolve subcategory name:', e);
            }

            // Fetch products
            try {
                const firstPage = await getProductsBySubcategory(subcategoryId, 1);
                    ...
                    if (totalPages > 1) {
                        for (let p = 2; p <= totalPages; p += 5) {
                            ...
                            const results = await Promise.allSettled(
                                chunk.map(pg => getProductsBySubcategory(subcategoryId, pg))
                            );
                            results.forEach(r => {
        ...
        <div className="bg-gray-50 min-h-screen py-8 md:py-12">
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <div className="text-[12px] md:text-sm text-gray-500 mb-6 flex items-center gap-2 font-medium">
                    ...
                    <span>/</span>
                    <span className="text-brand-orange font-bold capitalize">{subcategoryName || 'Subcategory'}</span>
                </div>

                <h1 className="text-2xl md:text-4xl font-black text-gray-900 mb-8 capitalize">{subcategoryName || 'Products'}</h1>
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 bg-gray-50 rounded-2xl border border-gray-200 border-dashed">
                        <p className="text-gray-400 font-medium">No products found in this subcategory.</p>
                    </div>
                )}
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2 mt-10">
                        {Array.from({ length: totalPages }, (_, i) => {
                            ...
                            return (
                                <Link
                                    key={pageNum}
                                    href={`/subcategory/${subcategoryId}?page=${pageNum}`}
                                    scroll={true}
