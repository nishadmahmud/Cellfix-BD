                    ...
                    {timelineStages.map((stage) => {
                        ...
                        return (
                            <div key={stage.id} className="relative flex flex-col items-center z-10">
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${isCompleted ? "bg-gradient-to-br from-brand-orange to-orange-500 text-white shadow-lg shadow-orange-500/30" : "bg-white border-2 border-gray-300 text-gray-400"} ${isCurrent ? "ring-4 ring-orange-100 scale-110" : ""}`}>
                    ...
                    return (
                        <div key={stage.id} className="flex items-start gap-4">
                            <div className="flex flex-col items-center">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isCompleted ? "bg-gradient-to-br from-brand-orange to-orange-500 text-white shadow-md" : "bg-white border-2 border-gray-300 text-gray-400"} ${isCurrent ? "ring-3 ring-orange-100" : ""}`}>
                                    {isCompleted ? <CheckCircle2 className="w-5 h-5" /> : <span className="text-sm">{stage.id}</span>}
                                </div>
                                {!isLast && <div className={`w-0.5 h-8 ${isCompleted ? "bg-brand-orange" : "bg-gray-200"}`} />}
                            </div>
                            <div className={`flex items-center gap-2 pt-2 ${isCompleted ? "text-gray-900" : "text-gray-400"}`}>
                                <StageIcon className={`w-5 h-5 ${isCompleted ? "text-brand-orange" : ""}`} />
        ...
        <div className="min-h-screen bg-gray-50 pb-20 md:pb-10">
            ...
            <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
                <div className="max-w-4xl mx-auto px-4 md:px-8 py-10 md:py-16 text-center">
                    <div className="w-16 h-16 md:w-20 md:h-20 bg-brand-orange/20 rounded-2xl flex items-center justify-center mx-auto mb-5 backdrop-blur-sm border border-brand-orange/20">
                    ...
                    <form onSubmit={handleTrack} className="flex flex-col sm:flex-row gap-3">
                        <div className="flex-1 relative">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange transition-all text-sm md:text-base"
                                style={{ fontSize: "16px" }}
                            />
                        </div>
                        <button
                            type="submit"
                            disabled={loading}
                            className="px-8 py-4 bg-brand-orange text-white font-extrabold rounded-xl shadow-lg shadow-brand-orange/30 hover:bg-[#ff1a2b] hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed whitespace-nowrap"
                        >
                            {loading ? (
                    ...
                    <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
                        ...
                        {[5, 6].includes(Number(orderData.tran_status || orderData.status)) && (
                            <div className="px-5 md:px-8 py-8">
                                <div className={`py-8 px-6 text-center rounded-xl ${Number(orderData.tran_status || orderData.status) === 5 ? "bg-red-50" : "bg-orange-50"}`}>
                                    <h3 className={`text-xl font-bold mb-2 ${Number(orderData.tran_status || orderData.status) === 5 ? "text-red-700" : "text-orange-700"}`}>
                                        {Number(orderData.tran_status || orderData.status) === 5 ? "Order Canceled" : "Order On Hold"}
                                    </h3>
                                    <p className={`text-sm ${Number(orderData.tran_status || orderData.status) === 5 ? "text-red-600" : "text-orange-600"}`}>
                                    ...
                                    {orderData.sales_details.map((item, i) => (
                                        <div key={i} className="flex gap-4 p-3 bg-gray-50 rounded-xl">
                                            ...
                                            <div className="text-right flex-shrink-0">
                                                <p className="font-bold text-brand-orange">৳{item.price * item.qty}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="px-5 md:px-8 pb-8">
                            <div className="p-4 bg-gray-50 rounded-xl space-y-2 text-sm">
                                ...
                                <div className="flex justify-between pt-2 border-t border-gray-200 font-bold text-lg"><span>Grand Total</span><span className="text-brand-orange">৳{(Number(orderData.sub_total ?? orderData.total ?? 0) + Number(orderData.delivery_fee ?? 0) - Number(orderData.coupon_discount ?? 0))}</span></div>
                            </div>
                        </div>
                    </div>
                )}
