            ...
            <div className="max-w-7xl mx-auto px-3 md:px-6">
                <div className="text-center mb-6 md:mb-14">
                    <h2 className="text-xl md:text-4xl font-extrabold text-gray-900 mb-2 md:mb-4 tracking-tight">
                        Best <span className="text-brand-orange">Deals</span>
                    </h2>
                    ...
                    {displayDeals.map((deal) => (
                        <div key={deal.id} className="group bg-white rounded-xl md:rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col sm:flex-row">
                            <div className="sm:w-2/5 aspect-[16/9] sm:aspect-auto relative overflow-hidden bg-gray-100">
                                <Image src={deal.imageUrl || "/no-image.svg"} alt={deal.title || "Deal Image"} fill unoptimized className="object-cover object-center group-hover:scale-105 transition-transform duration-500" />
                            ...
                            <div className="sm:w-3/5 p-4 md:p-8 flex flex-col justify-center">
                                <h3 className="text-base md:text-2xl font-extrabold text-gray-900 mb-1 md:mb-3 leading-tight group-hover:text-brand-orange transition-colors">{deal.title}</h3>
                                ...
                                <div className="flex items-end gap-2 md:gap-3 mb-2 md:mb-4">
                                    <span className="text-xl md:text-3xl font-black text-brand-orange">{deal.price}</span>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <Link href={deal.link || "/shop"} className="text-[10px] md:text-sm font-bold text-brand-orange hover:text-orange-700 underline underline-offset-4 transition-colors">Shop Now →</Link>
                                </div>
                            </div>
                        </div>
                    ))}
