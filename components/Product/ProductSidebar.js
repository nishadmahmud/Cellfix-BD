...
export default function ProductSidebar() {
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div className="bg-white border text-left border-gray-200 rounded-xl p-4 flex gap-4 items-center shadow-sm">
                <div className="bg-orange-50 text-brand-orange p-3 rounded-full shrink-0">
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mt-4 shadow-sm">
                ...
                <div className="p-4 flex flex-col gap-3">
                    {warranties.map(warranty => (
                        <label key={warranty.id} className={`flex items-start gap-3 p-3 border rounded-xl cursor-pointer transition-colors ${selectedWarranties.includes(warranty.id) ? 'border-brand-orange bg-orange-50/30' : 'border-gray-200 hover:border-gray-300'}`}>
                            <div className={`mt-0.5 w-5 h-5 shrink-0 rounded flex items-center justify-center border transition-colors ${selectedWarranties.includes(warranty.id) ? 'bg-brand-orange border-brand-orange' : 'bg-white border-gray-300'}`}>
                ...
                <div className="bg-gray-50 px-4 py-3 border-t border-gray-100 flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-600 font-medium tracking-wide">I agree to CellFix's <span className="underline cursor-pointer">terms & conditions</span></span>
                </div>
            </div>
        </div>
    );
