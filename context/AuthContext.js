...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        try {
            const storedToken = localStorage.getItem("cellfix_token");
            const storedUser = localStorage.getItem("cellfix_user");
            ...
            localStorage.removeItem("cellfix_token");
            localStorage.removeItem("cellfix_user");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const persistAuth = useCallback((newToken, newUser) => {
        try {
            localStorage.setItem("cellfix_token", newToken);
            localStorage.setItem("cellfix_user", JSON.stringify(newUser));
        } catch (error) {
    ...
    const updateProfile = useCallback(async (profileData) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setUser(updatedUser);
                try {
                    localStorage.setItem("cellfix_user", JSON.stringify(updatedUser));
                } catch (e) {
